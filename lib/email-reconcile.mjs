...
/**
 * The e-mails of the window: both searches, deduplicated by thread, each
 * reduced to its latest message not sent by `account`.
 *
...
export async function fetchEmails({ account, days = DEFAULT_DAYS, maxPerQuery = 500, concurrency = 8, run = (args) => runGog(account, args) }) {
  ...
  const emails = await mapLimit([...recortes.keys()], concurrency, async (id) => {
    ...
    const inbound = messages.filter((m) => parseAddress(m?.headers?.from).address !== self);
    const last = inbound.sort((a, b) => Number(a.internalDate) - Number(b.internalDate)).at(-1);
    if (!last) return null;
    const { name, address } = parseAddress(last.headers?.from);
    const at = new Date(Number(last.internalDate));
    return {
      threadId: id,
      recortes: recortes.get(id),
      fromName: name,
      address,
      subject: unwrapUntrusted(last.headers?.subject),
      date: localToday(at),
      dateIso: at.toISOString(),
      body: unwrapUntrusted(last.body || last.snippet).slice(0, BODY_LIMIT),
    };
  });
  return { emails: emails.filter(Boolean), searched };
}
...
/** Newest entry per row (or, unmatched, per sender) and `what`. Input is oldest first. */
function latestPer(entries, what = () => '') {
  const by = new Map();
  for (const e of entries) by.set(`${e.row ? rowKey(e.row.track, e.row.num) : e.sender}\0${what(e)}`, e);
  return [...by.values()];
}
 ...
 *
 * Pending actions and recruiter requests keep the newest e-mail per row (or
 * per sender when unmatched); a pending action whose deadline has passed, or
 * whose row ends closed, is dropped.
 *
...
export function planReconciliation(judgments, { threshold = MATCH_THRESHOLD, today = localToday() } = {}) {
  ...
  for (const j of ordered) {
    ...
    const listed = { ...summary, deadline, row: confident ? row : null, sender: (email.fromName || email.address).toLowerCase() };
  ...
  const pendingActions = latestPer(actions, (a) => a.action)
  ...
  const recruiterRequests = latestPer(requests).sort((a, b) => b.date.localeCompare(a.date));
