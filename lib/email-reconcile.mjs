...
export function refreshAppliedPlan(plan, tracks) {
  ...
  const statuses = new Map(tracks.flatMap((track) => readTrackerRows(track.root).rows.map((row) => [rowKey(track.id, row.num), row.canonical])));
  const statusOf = (row) => statuses.get(rowKey(row.track, row.num));
  return {
    ...plan,
