 ...
 *   - Never invent personal information. Match/pick/bool return the NONE/null
 *     outcome when the profile does not answer, or when Jev is below the
 *     confidence threshold, or when Jev is disabled — never a guess.
 *     `readyToSubmit` abstains toward the filled-required floor instead (see
 *     that function).
 ...
 * consistent with the candidate's profile. Jev's answer then falls into one of
 * three outcomes:
 *   - confident consistent (probability > 0.5, confidence >= threshold) -> ready.
 *   - confident inconsistent (probability <= 0.5, confidence >= threshold) -> not ready.
 *   - abstained (confidence < threshold), disabled, or a missing/malformed
 *     probability -> Jev made NO decision, which is not the same as "not
 *     ready": the deterministic floor already passed, so the form IS ready,
 *     and `abstained: true` tells the caller Jev did not judge it. A real
 *     transport error is a communication failure, not a Jev judgment:
 *     `ready: false` with `error` set and `abstained: true`.
 *
 * @param {Array<{label?: string, required?: boolean, value?: string}>|{fields: Array, profileFacts?: object}} fieldsState
 * @param {{jev?: typeof jevNoul, threshold?: number}} [opts]
 * @returns {Promise<{ready: boolean, confidence: number, error: string|null, abstained: boolean}>}
 */
export async function readyToSubmit(fieldsState, { jev = jevNoul, threshold } = {}) {
  ...
  if (anyRequiredEmpty) return { ready: false, confidence: 1, error: null, abstained: false };
  ...
  const answer = await jev({
    ...
    instructions:
      ...
      'Using ONLY the supplied profile facts, decide whether every filled value is consistent with them ' +
      'and nothing looks fabricated or misplaced.',
  ...
  if (answer?.error) return { ready: false, confidence: 0, error: answer.error, abstained: true };
  // Disabled Jev: fall back to the deterministic floor (all required fields are filled).
  if (answer?.enabled === false) return { ready: true, confidence: 0, error: null, abstained: true };

  const probability = answer?.probability;
  // No usable probability (malformed answer): Jev made no decision — abstain to the floor.
  if (typeof probability !== 'number') return { ready: true, confidence: 0, error: null, abstained: true };

  const confidence = noulConfidence(probability);
  // Below threshold: Jev abstained — "no decision", not "not ready". Fall back to the floor.
  if (confidence < limit) return { ready: true, confidence, error: null, abstained: true };

  return { ready: probability > 0.5, confidence, error: null, abstained: false };
}
