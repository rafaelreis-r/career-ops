...
| `reconcile-email.mjs` | Reconciles every track's tracker (shared `trilhas.yml`) with Gmail through `gog` (read-only) and Jev: proposes forward-only status changes, lists recruiter requests, pending actions with deadlines, live processes and low-confidence matches. Dry-run by default; `--apply` backs up and writes through `set-status.mjs` |
...
| Asks to reconcile the tracker with e-mail ("reconciliar com email") | `node reconcile-email.mjs` (dry-run), show its lists, then `--apply` only after the user approves the proposed changes |
...
- **NEVER submit an application without the user reviewing it first.** Fill forms, draft answers, generate PDFs -- but always STOP before clicking Submit/Send/Apply. The user makes the final call. Exceptions: a worker already dispatched on `apply-llm-owned` submits once after its ready gate (see `modes/apply-llm-owned.md`), and `web/scripts/apply-hybrid.mjs` submits once after its pre-submit gate clears, by user instruction of 2026-09-23; interactive `apply` is unchanged.
